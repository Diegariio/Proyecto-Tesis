...
@section('title', 'Registrar Tratamiento Radioterapia')


@section('content')
<h1 class="h4">Registrar Tratamiento Radioterapia</h1>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fechaComite = document.getElementById('fecha_comite');
    const fechaSimulacion = document.getElementById('fecha_simulacion');
    const fechaInicio = document.getElementById('fecha_inicio');
    const fechaTermino = document.getElementById('fecha_termino');

    // Helper para sumar días a una fecha (formato yyyy-mm-dd)
    ...
    if (fechaIndicacion && fechaComite && fechaSimulacion && fechaInicio && fechaTermino) {
        fechaIndicacion.addEventListener('change', function () {
            if (fechaIndicacion.value) {
                fechaComite.min = sumarDias(fechaIndicacion.value, 1);
                if (fechaComite.value && fechaComite.value < fechaComite.min) {
                    fechaComite.value = '';
                }
                // Limpiar y resetear los siguientes
                fechaSimulacion.value = '';
                fechaSimulacion.min = '';
                fechaInicio.value = '';
                fechaInicio.min = '';
                fechaTermino.value = '';
        ...
        fechaComite.addEventListener('change', function () {
            if (fechaComite.value) {
                fechaSimulacion.min = sumarDias(fechaComite.value, 1);
                if (fechaSimulacion.value && fechaSimulacion.value < fechaSimulacion.min) {
                    fechaSimulacion.value = '';
                }
                // Limpiar y resetear los siguientes
                fechaInicio.value = '';
        ...
        fechaSimulacion.addEventListener('change', function () {
            if (fechaSimulacion.value) {
                fechaInicio.min = sumarDias(fechaSimulacion.value, 1);
                if (fechaInicio.value && fechaInicio.value < fechaInicio.min) {
                    fechaInicio.value = '';
                }
                // Limpiar y resetear el siguiente
                fechaTermino.value = '';
                fechaTermino.min = '';
            } else {
                fechaInicio.min = '';
        ...
        fechaInicio.addEventListener('change', function () {
            if (fechaInicio.value) {
                fechaTermino.min = sumarDias(fechaInicio.value, 1);
